...
#       $WallpaperBlobFiles = $AzureStorageBlobContent | Where-Object { $PSItem.Name -match "^img(\d?[1-9]|[1-9]0).jpg$" }
        $WallpaperBlobFiles = $AzureStorageBlobContent | Where-Object { $PSItem.Name -match "^img(\d?[1-9]|[1-9]0).png$" }
        if ($WallpaperBlobFiles -ne $null) {
            ...
            # Remove all items in '%windir%\Web\Wallpaper\Flowers' (Windows 10) directory and replace with wallpaper content from storage account
            $ThemeWallpaperImagePath = Join-Path -Path $env:windir -ChildPath "Web\Wallpaper\Flowers"
#           $ThemeWallpaperImages = Get-ChildItem -Path $ThemeWallpaperImagePath -Filter "*.jpg"
            $ThemeWallpaperImages = Get-ChildItem -Path $ThemeWallpaperImagePath -Filter "*.png"
